...
const Magasine = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get('http://192.168.248.1:8000/article/get');
  ...
  const fetchArticlesByCategory = async (category) => {
    try {
      ...
      const response = await axios.get(`http://192.168.248.1:8000/article/getByCategory/${category}`);
